                ...
                <div class="card mt-1">
                    <div class="card-header">
                        <a href="{{ url()->previous() }}" class="h2 text-decoration-none text-primary">
                            ...
                            <span> Chỉnh sửa chủ đề câu hỏi</span>
                        </a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('teacher.topics.update', $topic->id) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="typeOfClassId" name="typeOfClassId">
                                    @foreach($typeClasses as $typeClass)
                                        <option @if ($topic->type_class_id == $typeClass->id) selected @endif value="{{$typeClass->id}}">{{$typeClass->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" id="description"
                                          ...
                                          class="form-control text-secondary p-3">{{$topic->description}}</textarea>
                            </div>
